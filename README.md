...
一个面向口播视频的 Agent Skill，根据字级逐字稿添加网感缩放、字幕强调、普通烧录字幕、逐条累积清单、渐进关键词散布和定时图片贴图，并提供可逐字调整的本地审查网页
...
- 在连续词索引对应的时间内显示本地图片，支持 PNG、JPG、JPEG、WebP、BMP
- 任意尺寸图片固定按 `contain` 保持宽高比，审查页支持拖动位置和右下角缩放，最终 MP4 共用同一布局
- 保持预览与 FFmpeg 最终渲染一致
...
结构化清单、关键词散布和图片贴图使用 `overlays.json` v2，可以由 Agent 识别范围后批量导入
